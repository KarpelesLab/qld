//! RISC-V 64 ELF tests (workstream W30).
//!
//! Every test links the same objects with qld and with lld and compares
//! what the two produced *symbolically*: each address an instruction
//! computes — an `auipc`/`lui` pair, a branch, a `tp` offset, a GOT load —
//! is printed as `symbol+offset` (a GOT slot by the dynamic relocation that
//! fills it or the value it holds), so the comparison holds wherever each
//! linker placed things. Dynamic relocations and tags, symbol sizes, line
//! tables, `.eh_frame` ranges and DWARF ranges are compared the same way.
//! qld follows lld's relaxation decisions, so relaxed code must match
//! instruction for instruction, and label-difference tables byte for byte.
//!
//! The objects come from `clang --target=riscv64-linux-gnu` and are
//! freestanding: nothing here needs a RISC-V sysroot or can run the output
//! (the `riscv64-*` fixtures do, under qemu, in CI).
//!
...
fn probe() -> Result<Tools, String> {
    ...
    if !String::from_utf8_lossy(&version.stdout).contains("riscv64") {
    ...
    let targets = Command::new(&cc)
        ...
        .output()
        .map_err(|e| e.to_string())?;
    if !String::from_utf8_lossy(&targets.stdout).contains("riscv64") {
...
/// RV64GC/LP64D defaults plus `extra`, into `object`.
...
fn dyn_relocs(tools: &Tools, dir: &Path, file: &str) -> Vec<(u64, String, String)> {
    ...
    for line in run_ok(dir, &tools.readelf, &["-rW", file]).lines() {
        ...
        if !dynamic || place.len() != 16 || !kind.starts_with("R_RISCV_") {
            continue;
        }
        let Some(place) = hex(place) else { continue };
        let rest: Vec<&str> = match rest.first() {
            Some(value) if value.len() == 16 && rest.len() > 1 => rest.get(1..).unwrap().to_vec(),
            _ => rest.to_vec(),
        };
...
fn listing(tools: &Tools, dir: &Path, file: &str) -> String {
    ...
    for line in text.lines() {
            ...
            && let Some(offset) = imm(offset)
        {
            ...
            let text = if let Some(value) = regs.get(&ops[1]) {
                let target = value.wrapping_add(offset as u64);
...
void _start(void) {
  ...
  int r = switchy(3) + aligned_fn(2) + tail(4) + (int)strs[1][0];
...
fn static_links_match_lld() {
    ...
    compile(tools, &dir, "main.c", MAIN_C, "main.o", &[]);
    ...
    for (name, args) in [
        ("relaxed", &["-static", "main.o", "other.o", "far.o"][..]),
        (
            "norelax",
            &["-static", "main-norelax.o", "other.o", "far.o"],
        ),
        (
            "ld-norelax",
            &["-static", "--no-relax", "main.o", "other.o", "far.o"],
        ),
        ("align", &["-static", "main-align.o", "other.o", "far.o"]),
        ("norvc", &["-static", "main-norvc.o", "other.o", "far.o"]),
    ] {
        let (lld, ours) = link_both(tools, &dir, name, args);
        assert_same(tools, &dir, &lld, &ours);
    }
    // Relaxation happened: the near tail call is a `c.j`, the far call
    // kept its `auipc`, and a local-exec address fitting 12 bits is `tp`
    // plus an immediate.
    let code = listing(tools, &dir, "relaxed.qld");
    ...
    let header = run_ok(&dir, &tools.readelf, &["-h", "relaxed.qld"]);
    assert!(header.contains("RVC, double-float ABI"), "{header}");
}
...
fn dynamic_links_match_lld() {
    ...
    // The PLT is the psABI's, and TLS descriptors relax: to initial-exec
    // against a library's variable, to local-exec for the executable's.
    let pie = listing(tools, &dir, "piedesc.qld");
    assert!(pie.contains("GOT[R_RISCV_TLS_TPREL64 lib_tls"), "{pie}");
...
/// Assembly with calls and tail calls around the `jal` and `c.j` limits,
/// alignment directives, local-exec TLS accesses, branches over relaxed
/// calls and label differences in data.
...
fn attributes_are_merged() {
    ...
    let arch = |file: &str| {
        run_ok(&dir, &tools.readelf, &["-A", file])
            .lines()
            .find(|l| l.contains("TagName: arch"))
            .map(str::to_string)
            .and_then(|_| {
                run_ok(&dir, &tools.readelf, &["-A", file])
                    .lines()
                    .find(|l| l.trim_start().starts_with("Value: rv64"))
                    .map(|l| l.trim().to_string())
            })
            .unwrap_or_default()
    };
    let expected = arch(&lld);
    assert!(expected.contains("zba"), "{expected}");
    assert_eq!(arch(&ours), expected);
...
#[test]
fn incompatible_float_abis_are_rejected() {
    let tools = require!();
    let dir = scratch("abi");
    compile(
        tools,
    ...
    fs::write(dir.join("b.c"), "int b(void) { return 2; }\n").unwrap();
    run_ok(
        &dir,
        &tools.cc,
        &[
...
fn emitted_relocs(tools: &Tools, dir: &Path, file: &str) -> Vec<String> {
    ...
    for line in text.lines() {
        ...
        let Some(place) = hex(place).filter(|_| in_text && place.len() == 16) else {
            continue;
        };
...
fn other_output_kinds_match_lld() {
    ...
    for (name, args) in [
        (
            "gc",
            &["-static", "--gc-sections", "main.o", "other.o", "far.o"][..],
        ),
        (
            "script",
            &["-static", "-T", "script.ld", "main.o", "other.o", "far.o"],
        ),
    ] {
        let (lld, ours) = link_both(tools, &dir, name, args);
        assert_same(tools, &dir, &lld, &ours);
    ...
    let (lld, ours) = link_both(
        ...
        &dir,
        "emit",
        &["-static", "--emit-relocs", "main.o", "other.o", "far.o"],
    );
    assert_same(tools, &dir, &lld, &ours);
    ...
    // `-r` keeps the ABI flags and merges the attributes; its output links
    // as the objects did.
    run_ok(
        &dir,
        Path::new(env!("CARGO_BIN_EXE_qld")),
        &["-r", "main.o", "other.o", "-o", "partial.o"],
    );
    let header = run_ok(&dir, &tools.readelf, &["-h", "partial.o"]);
    assert!(header.contains("RVC, double-float ABI"), "{header}");
    let attributes = run_ok(&dir, &tools.readelf, &["-A", "partial.o"]);
    assert_eq!(
        attributes.matches("TagName: arch").count(),
        1,
        "{attributes}"
    );
